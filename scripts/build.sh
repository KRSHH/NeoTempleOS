...
function chroot_exit_teardown() {
    # Use lazy unmount (-l) to handle busy mount points
    sudo chroot chroot umount /proc || sudo chroot chroot umount -l /proc || true
    sudo chroot chroot umount /sys || sudo chroot chroot umount -l /sys || true
    sudo chroot chroot umount /dev/pts || sudo chroot chroot umount -l /dev/pts || true
    sudo umount chroot/dev || sudo umount -l chroot/dev || true
    sudo umount chroot/run || sudo umount -l chroot/run || true
}
